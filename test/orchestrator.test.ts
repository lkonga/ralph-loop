...
describe("runPreCompleteChain", () => {
	...
	it("previousResults accumulates across hooks", async () => {
		...
		const service = createMockHookService(async (input) => {
			receivedPreviousResults.push(
				input.previousResults ? [...input.previousResults] : undefined,
			);
			return { action: "continue" };
		});
...
describe("shouldRetryError", () => {
	...
	it("returns false for transient error at retry cap", () => {
		expect(
			shouldRetryError(new Error("network error"), MAX_RETRIES_PER_TASK),
		).toBe(false);
	});

	it("returns false for fatal (non-transient) error", () => {
		expect(
			shouldRetryError(new Error("Cannot read properties of undefined"), 0),
		).toBe(false);
	});
...
describe("LoopOrchestrator.injectContext", () => {
	...
	it("injectContext sets pendingContext that is consumed by buildPrompt", () => {
		...
		expect((orch as any).pendingContext).toBe(
			"The bug is in parser.ts line 42",
		);
	});
...
describe("Security rejection as feedback", () => {
	...
	it("orchestrator continues loop when session hook is blocked", async () => {
		...
		const commandBlockedEvents = events.filter(
			(e) => e.kind === LoopEventKind.CommandBlocked,
		);
...
describe("runBearings", () => {
	...
	it("returns healthy when both tsc and vitest pass", async () => {
		...
		const result = await runBearings(
			"/tmp",
			noopLogger,
			{ enabled: true, runTsc: true, runTests: true },
			execFn,
		);
	...
	it("returns healthy when no tsconfig.json or vitest config exists (skips checks)", async () => {
		...
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "ralph-bearings-noconfig-"),
		);
		const execFn = async () => ({
			exitCode: 1,
	...
	it("returns unhealthy when tsc fails", async () => {
		...
		const os = require("os");
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "ralph-bearings-tsc-"),
		);
		...
		const execFn = async (cmd: string) => {
			if (cmd.includes("tsc"))
				return { exitCode: 1, output: "error TS2345: Argument of type..." };
	...
	it("returns unhealthy when vitest fails", async () => {
		...
		const os = require("os");
		const tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "ralph-bearings-vitest-"),
		);
		fs.writeFileSync(path.join(tmpDir, "tsconfig.json"), "{}", "utf-8");
		fs.writeFileSync(path.join(tmpDir, "vite.config.ts"), "", "utf-8");
		const execFn = async (cmd: string) => {
			if (cmd.includes("vitest"))
				return { exitCode: 1, output: "3 tests failed" };
			return { exitCode: 0, output: "" };
		};
		const result = await runBearings(
			tmpDir,
		...
		expect(result.issues.length).toBeGreaterThan(0);
		expect(
			result.issues.some((i) => i.includes("test") || i.includes("Test")),
		).toBe(true);
		expect(result.fixTask).toContain("Fix baseline");
	});
});
...
describe("Bearings phase integration", () => {
	...
	it("healthy bearings proceed to task (BearingsChecked emitted)", async () => {
		...
		await orch.start();
		const bearingsChecked = events.filter(
			(e) => e.kind === LoopEventKind.BearingsChecked,
		);
		...
		const taskStarted = events.filter(
			(e) => e.kind === LoopEventKind.TaskStarted,
		);
	...
	it("unhealthy bearings inserts fix task and yields BearingsFailed after fix attempt", async () => {
		...
		const bearingsFailedEvents = events.filter(
			(e) => e.kind === LoopEventKind.BearingsFailed,
		);
	...
	it("bearings disabled skips check", async () => {
		...
		const bearingsFailed = events.filter(
			(e) => e.kind === LoopEventKind.BearingsFailed,
		);
...
describe("Bearings policy split", () => {
	...
	it("task start no longer implies full vitest by default (perTask=none)", async () => {
		...
		orch.bearingsExecFn = async (cmd: string) => {
			calls.push(cmd);
			return { exitCode: 0, output: "" };
		};
		(orch as any).executionStrategy = {
...
describe("CHECKPOINT: Bearings Policy Verification (Task 110)", () => {
	...
	beforeEach(() => {
		...
		tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "ralph-bearings-checkpoint-"),
		);
	});
	...
	it("checkpoint path still triggers configured validation (bearings at checkpoint level)", async () => {
		...
		const bearingsChecked = events.filter(
			(e: any) => e.kind === LoopEventKind.BearingsChecked,
		);
		expect(bearingsChecked.length).toBeGreaterThanOrEqual(1);
	});
});
...
describe("Async Verification Runner (Task 111)", () => {
	...
	it("BearingsExecFn returns a Promise with exitCode/output shape", async () => {
		const asyncExec = async (cmd: string, cwd: string) => ({
			exitCode: 0,
			output: "ok",
		});
	...
	it("long-running verification does not block subsequent event emissions", async () => {
		...
		const bearingsPromise = runBearings(
			"/tmp",
			noopLogger,
			{ enabled: true, runTsc: true, runTests: true },
			slowExec,
...
describe("CHECKPOINT: Non-Blocking Verification (Task 112)", () => {
	...
	it("progress log emitted before tsc exec completes", async () => {
		...
		await runBearings(
			dir,
			timelineLogger,
			{ enabled: true, runTsc: true, runTests: false },
			execFn,
			"tsc",
		);
		fs.rmSync(dir, { recursive: true, force: true });
		...
		const logIdx = timeline.findIndex(
			(e) => e.startsWith("log:") && e.includes("tsc"),
		);
...
describe("CHECKPOINT: Cache / Dirty-Skip Verification (Task 114)", () => {
	...
	it("repeated start reuses cached green state — zero tsc calls on second run", async () => {
		...
		await orch2.start();
		const secondRunTscCalls = calls
			.slice(callsBefore)
			.filter((c) => c.includes("tsc")).length;
	...
	it("dirty change to tracked file invalidates cache and triggers full verification", async () => {
		...
		(orch as any).executionStrategy = {
			execute: async (task: any) => {
				...
				fs.writeFileSync(
					path.join(tmpDir, "PRD.md"),
					prd.replace(`- [ ] ${task.description}`, `- [x] ${task.description}`),
					"utf-8",
				);
				return {
					completed: true,
...
describe("Bearings lifecycle events (Task 115)", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "ralph-bearings-lifecycle-"),
		);
	});
	...
	it("emits BearingsStarted, BearingsProgress, BearingsCompleted in correct order on healthy run", async () => {
		...
		const started = events.filter(
			(e) => e.kind === LoopEventKind.BearingsStarted,
		);
		const progress = events.filter(
			(e) => e.kind === LoopEventKind.BearingsProgress,
		);
		const completed = events.filter(
			(e) => e.kind === LoopEventKind.BearingsCompleted,
		);
	...
	it("emits BearingsSkipped when cache hit skips verification", async () => {
		...
		const skipped = events.filter(
			(e) => e.kind === LoopEventKind.BearingsSkipped,
		);
		...
		expect(
			skipped.length +
				events.filter((e) => e.kind === LoopEventKind.BearingsCompleted).length,
...
describe("CHECKPOINT: Startup DX Verification (Task 116)", () => {
	...
	it("end-to-end startup uses new bearings policy: tsc-only startup, none per-task, no vitest swarm", async () => {
		...
		const bearingsStarted = events.filter(
			(e) => e.kind === LoopEventKind.BearingsStarted,
		);
		...
		const bearingsCompleted = events.filter(
			(e) => e.kind === LoopEventKind.BearingsCompleted,
		);
	...
	it("logs clearly explain what is running during startup bearings", async () => {
		...
		const tscLog = logs.some(
			(l) => l.includes("Bearings") && l.includes("tsc"),
		);
	...
	it("logs clearly explain why bearings are skipped (per-task=none)", async () => {
		...
		expect(skipped.length).toBeGreaterThanOrEqual(1);
		const hasReasonExplained = skipped.some(
			(e: any) => typeof e.reason === "string" && e.reason.length > 0,
		);
	...
	it("featureBranch config has no protectedBranches property", () => {
		expect(DEFAULT_CONFIG.featureBranch).not.toHaveProperty(
			"protectedBranches",
		);
	});
	...
	it("package.json has no featureBranch.protectedBranches setting", () => {
		...
		expect(settings).not.toHaveProperty(
			"ralph-loop.featureBranch.protectedBranches",
		);
	});
});
...
describe("Startup branch gate (linear flow)", () => {
	...
	it("creates branch from main — always creates new branch with hash suffix", async () => {
		...
		expect(gitOps.createAndCheckoutBranch).toHaveBeenCalledWith(
			tmpDir,
			"ralph/my-feature-project-abc1234",
		);
		const branchCreated = events.find(
			(e) => e.kind === LoopEventKind.BranchCreated,
		);
	...
	it("creates branch from bisect/v0.39-lean", async () => {
		...
		expect(gitOps.createAndCheckoutBranch).toHaveBeenCalledWith(
			tmpDir,
			"ralph/my-feature-project-def5678",
		);
		const branchCreated = events.find(
			(e) => e.kind === LoopEventKind.BranchCreated,
		);
	...
	it("creates branch from any arbitrary branch", async () => {
		const gitOps = await import("../src/gitOps");
		vi.spyOn(gitOps, "getCurrentBranch").mockResolvedValue(
			"feature/other-work",
		);
		...
		expect(gitOps.createAndCheckoutBranch).toHaveBeenCalledWith(
			tmpDir,
			"ralph/my-feature-project-fed9876",
		);
		const branchCreated = events.find(
			(e) => e.kind === LoopEventKind.BranchCreated,
		);
	...
	it("skipped when featureBranch.enabled is false", async () => {
		const gitOps = await import("../src/gitOps");
		const getCurrentBranchSpy = vi
			.spyOn(gitOps, "getCurrentBranch")
			.mockResolvedValue("main");
		const createSpy = vi
			.spyOn(gitOps, "createAndCheckoutBranch")
		...
		expect(
			events.find((e) => e.kind === LoopEventKind.BranchEnforcementFailed),
		).toBeUndefined();
		expect(
			events.find((e) => e.kind === LoopEventKind.BranchCreated),
		).toBeUndefined();
	});
	...
	it("logs warning but does not crash when switch-back fails", async () => {
		...
		vi.spyOn(gitOps, "checkoutBranch").mockResolvedValue({
			success: false,
			error: "conflict",
		});
		...
		expect(
			events.find((e) => e.kind === LoopEventKind.BranchSwitchedBack),
		).toBeUndefined();
...
describe("LaneScheduler", () => {
	...
	it("refresh re-reads PRDs and detects completed tasks", () => {
		...
		fs.writeFileSync(
			path.join(tmpDir, "repo-a", "PRD.md"),
			"- [x] Task 1\n- [ ] Task 2\n",
		);
		scheduler.refresh();
	...
	it("skips disabled lanes", () => {
		const laneA = makeLane("repo-a", "- [ ] Task A1\n");
		const laneB: RepoLane = {
			repoId: "repo-b",
			workspaceFolder: path.join(tmpDir, "repo-b"),
			prdPath: "PRD.md",
			progressPath: "progress.txt",
			enabled: false,
		};
...
describe("LaneBranchManager", () => {
	...
	it("switchBackAll switches each lane back independently", async () => {
		...
		expect(gitOps.checkoutBranch).toHaveBeenCalledWith(
			laneA.workspaceFolder,
			"main",
		);
		expect(gitOps.checkoutBranch).toHaveBeenCalledWith(
