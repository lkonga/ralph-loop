...
import {
	...
	type HookResult,
	type IConsistencyChecker,
	type ILogger,
	type IRalphHookService,
	type ITaskExecutionStrategy,
	type LoopEvent,
	LoopEventKind,
	LoopState,
...
export function resolveAgentMode(
	...
	if (!KNOWN_AGENTS.has(task.agent)) {
		logger.warn(
			`Unknown agent "${task.agent}" — falling back to default "${defaultAgentMode}"`,
		);
		return defaultAgentMode;
	}
	return modeId;
...
export function defaultBearingsExec(
	cmd: string,
	cwd: string,
	signal?: AbortSignal,
): Promise<{ exitCode: number; output: string }> {
...
export type BearingsProgressFn = (
	stage: string,
...
export async function runBearings(
	...
	const shouldRunTsc =
		config.runTsc && (effectiveLevel === "tsc" || effectiveLevel === "full");
...
export async function runPreCompleteChain(
	...
	for (const hook of hooks) {
		...
		if (result.action === "retry") {
			return { action: "retry", results };
		}
		if (result.action === "stop") {
			return { action: "stop", results };
		}
	}
...
export function startMonitor(
	...
	const interval = setInterval(() => {
		...
		if (staleIntervalCount >= config.stuckThreshold) {
			logger.warn(
				`Monitor: task ${taskId} appears stuck (${staleIntervalCount} stale intervals)`,
			);
			onEvent({
...
export class LoopOrchestrator {
	...
	constructor(
		...
		this.circuitBreakerChain = createDefaultChain(
			this.config.circuitBreakers,
			this.errorHashTracker,
		);
		this.consistencyChecker = consistencyChecker;
		const spConfig = this.config.sessionPersistence ?? {
			enabled: true,
		...
		try {
			for await (const event of this.runLoop()) {
				...
				if (LoopOrchestrator.STATE_CHANGE_EVENTS.has(event.kind)) {
					...
					this.onEvent({
						kind: LoopEventKind.StateNotified,
						state: stateStr,
						taskId,
					});
				}
				...
				if (
					event.kind === LoopEventKind.Stopped ||
			...
			if (this.originalBranch && this.activeBranch) {
				try {
					...
					if (result.success) {
						this.onEvent({
							kind: LoopEventKind.BranchSwitchedBack,
							from: this.activeBranch,
							to: this.originalBranch,
						});
					} else {
						this.logger.warn(
							`Failed to switch back to '${this.originalBranch}': ${result.error ?? "unknown error"}. Work remains on '${this.activeBranch}'.`,
						);
					}
				} catch (err: unknown) {
					const msg = err instanceof Error ? err.message : String(err);
					this.logger.warn(
						`Failed to switch back to '${this.originalBranch}': ${msg}. Work remains on '${this.activeBranch}'.`,
					);
				}
			}
		} finally {
	...
	setSessionId(sessionId: string | undefined): void {
		if (!this.config.features.useSessionTracking) {
			return;
		}
		...
		if (
			...
			this.pauseRequested = true;
			this.onEvent({
				kind: LoopEventKind.SessionChanged,
				oldSessionId: old,
	...
	private executionOptionsForTask(task?: { agent?: string }): ExecutionOptions {
		const agentMode = task
			? resolveAgentMode(
					task,
					this.config.agentMode ?? "ralph-executor",
					this.logger,
				)
			: this.config.agentMode;
		this.logger.log(
			`Agent mode resolved: ${agentMode} (config: ${this.config.agentMode}, task.agent: ${task?.agent ?? "none"})`,
		);
		return {
	...
	private async *runLoop(): AsyncGenerator<LoopEvent> {
		...
		const timeoutSignal = AbortSignal.timeout(
			this.config.inactivityTimeoutMs * effectiveMaxIterations,
		);
		this.linkedSignal = new LinkedCancellationSource(
		...
		try {
			...
			const stagnationConfig =
				this.config.stagnationDetection ?? DEFAULT_STAGNATION_DETECTION;
			const stagnationDetector = stagnationConfig.enabled
				? new StagnationDetector(
						stagnationConfig.hashFiles,
						stagnationConfig.maxStaleIterations,
					)
				: undefined;

			// Auto-decomposition
			const autoDecomposeConfig =
				this.config.autoDecompose ?? DEFAULT_AUTO_DECOMPOSE;
			const autoDecomposer = autoDecomposeConfig.enabled
				? new AutoDecomposer()
				: undefined;
			...
			const knowledgeManager = knowledgeConfig.enabled
				? new KnowledgeManager(
						knowledgeConfig.path,
						knowledgeConfig.maxInjectLines,
					)
				: undefined;

			// Struggle detector
			const struggleConfig =
				this.config.struggleDetection ?? DEFAULT_STRUGGLE_DETECTION;
			...
			if (sessionHook.additionalContext) {
				additionalContext = sessionHook.additionalContext;
			}
			if (sessionHook.blocked) {
				...
				yield {
					kind: LoopEventKind.CommandBlocked,
					command: this.config.hookScript ?? "unknown",
					reason: sessionHook.reason ?? "unknown",
					taskId: "",
				};
			...
			// Pre-flight PRD validation
			{
				...
				if (!validation.valid) {
					yield {
						kind: LoopEventKind.PrdValidationFailed,
			...
			if (featureBranchConfig?.enabled) {
				try {
					...
					if (!createResult.success) {
						yield {
							kind: LoopEventKind.BranchEnforcementFailed,
							reason: createResult.error ?? "branch creation failed",
						};
						return;
					}
					...
					this.logger.log(
						`Branch gate: created branch '${derivedName}' from '${currentBranch}'`,
					);
			...
			while (true) {
				...
				// Circuit breaker check before next iteration
				{
					...
					if (cbResult.tripped) {
						this.logger.log(
							`Circuit breaker tripped before next iteration: ${cbResult.reason}`,
						);
						yield {
							kind: LoopEventKind.CircuitBreakerTripped,
							breakerName: "",
							reason: cbResult.reason ?? "unknown",
							action: cbResult.action,
							taskInvocationId: "",
						};
						if (cbResult.action === "stop") {
							yield { kind: LoopEventKind.Stopped };
							return;
						}
						if (cbResult.action === "skip") {
							/* skip to next task */
						}
					}
				}
					...
					if (!iterationLimitExpanded) {
						...
						if (peekTask && !this.completedTasks.has(peekTask.id)) {
							...
							effectiveMaxIterations = Math.min(
								expanded,
								this.config.hardMaxIterations,
							);
							iterationLimitExpanded = true;
							this.logger.log(
								`Auto-expanded iteration limit: ${oldLimit} → ${effectiveMaxIterations}`,
							);
							yield {
								kind: LoopEventKind.IterationLimitExpanded,
								oldLimit,
								newLimit: effectiveMaxIterations,
							};
						} else {
							yield {
								kind: LoopEventKind.MaxIterations,
								limit: effectiveMaxIterations,
							};
							return;
						}
					} else {
				...
				if (
					...
					const readyTasks = pickReadyTasks(snapshot, concurrencyCap).filter(
					...
					if (readyTasks.length === 0) {
						...
						const parallelResults = await Promise.all(
							readyTasks.map(async (task) => {
								...
								this.onEvent({
									kind: LoopEventKind.TaskStarted,
									task,
									iteration,
									taskInvocationId: invId,
								});
								...
								try {
									...
									const prompt = buildPrompt(
										...
										progContent,
										20,
										this.config.promptBlocks,
										this.promptCapabilities,
										undefined,
										iteration,
										this.config.contextTrimming ?? DEFAULT_CONTEXT_TRIMMING,
										undefined,
										task.taskId,
										undefined,
										this.config.workspaceRoot,
									);
									const execResult = await this.executionStrategy.execute(
										task,
										prompt,
										this.executionOptionsForTask(task),
									);
									...
									if (execResult.completed) {
										...
										appendProgress(
											progressPath,
											`[${invId}] Task completed (parallel): ${task.description} (${Math.round(duration / 1000)}s)`,
										);
										this.onEvent({
											kind: LoopEventKind.TaskCompleted,
										...
										if (!pEditValidation.allowed) {
											this.logger.warn(
												`PRD write protection (parallel): ${pEditValidation.reason}`,
											);
											fs.writeFileSync(
												prdPath,
												prdContentBeforeParallel,
											...
											this.onEvent({
												kind: LoopEventKind.Error,
												message: `PRD write protection: ${pEditValidation.reason}`,
											});
										}

										// Atomic git commit per task (parallel path)
										const pCommitResult = await atomicCommit(
											this.config.workspaceRoot,
											task,
											invId,
										);
										if (pCommitResult.success) {
											appendProgress(
												progressPath,
												`[${invId}] Committed: ${pCommitResult.commitHash}`,
											);
											this.onEvent({
												kind: LoopEventKind.TaskCommitted,
												task,
												commitHash: pCommitResult.commitHash!,
												taskInvocationId: invId,
											});
										} else {
											this.logger.warn(
												`Atomic commit failed for parallel task ${task.id}: ${pCommitResult.error}`,
											);
											this.onEvent({
												kind: LoopEventKind.Error,
												message: `Atomic commit failed: ${pCommitResult.error}`,
											});
										}
									} else {
										appendProgress(
											progressPath,
											`[${invId}] Task timed out (parallel): ${task.description} (${Math.round(duration / 1000)}s)`,
										);
										this.onEvent({
											kind: LoopEventKind.TaskTimedOut,
											task: { ...task, status: TaskStatus.TimedOut },
											durationMs: duration,
											taskInvocationId: invId,
										});
									}
									...
									appendProgress(
										progressPath,
										`[${invId}] Task error (parallel): ${task.description} — ${msg}`,
									);
									this.onEvent({
										kind: LoopEventKind.Error,
										message: `Task "${task.description}" failed: ${msg}`,
									});
								}
							}),
						);
				...
				if (task.checkpoint) {
					...
					if (bearingsConfig.enabled) {
						...
						if (checkpointLevel !== "none") {
							const branch = VerificationCache.getGitBranch(
								this.config.workspaceRoot,
							);
							const treeHash = VerificationCache.getGitTreeHash(
								this.config.workspaceRoot,
							);
							const fileHashes = VerificationCache.computeFileHashes(
								this.config.workspaceRoot,
							);
							const cacheHit = verificationCache.isValid(
								this.config.workspaceRoot,
								branch,
								treeHash,
								checkpointLevel,
							...
							if (cacheHit) {
								this.logger.log(
									"Bearings (checkpoint): cache hit — skipping verification",
								);
								yield {
									kind: LoopEventKind.BearingsSkipped,
									reason: "checkpoint cache hit",
								};
								yield {
									kind: LoopEventKind.BearingsChecked,
									healthy: true,
									issues: [],
								};
							} else {
								yield {
									kind: LoopEventKind.BearingsStarted,
									level: checkpointLevel,
								};
								const cpStart = Date.now();
								const cbResult = await runBearings(
									this.config.workspaceRoot,
									this.logger,
									bearingsConfig,
									this.bearingsExecFn,
									...
									(stage, status) => {
										this.onEvent({
											kind: LoopEventKind.BearingsProgress,
											stage,
											status,
										});
									},
								...
								yield {
									kind: LoopEventKind.BearingsCompleted,
									healthy: cbResult.healthy,
									durationMs: cpDuration,
									issues: cbResult.issues,
								};
								yield {
									kind: LoopEventKind.BearingsChecked,
									healthy: cbResult.healthy,
								...
								if (!cbResult.healthy) {
									...
									yield {
										kind: LoopEventKind.BearingsFailed,
										issues: cbResult.issues,
									};
									this.pauseRequested = true;
									continue;
					...
					appendProgress(
						progressPath,
						`[${checkpointInvocationId}] [${task.taskId}] Checkpoint cleared: ${task.description}`,
					);
					yield {
						kind: LoopEventKind.TaskCompleted,
						task: { ...task, status: TaskStatus.Complete },
				...
				if (bearingsConfig.enabled && !skipBearingsOnce) {
					...
					if (bearingsLevel !== "none") {
						...
						if (cacheHit) {
							this.logger.log(
								"Bearings: cache hit — skipping verification (no relevant changes)",
							);
							yield {
								kind: LoopEventKind.BearingsSkipped,
								reason: "cache hit — no relevant changes",
							};
							yield {
								kind: LoopEventKind.BearingsChecked,
								healthy: true,
							...
							const bearingsResult = await runBearings(
								this.config.workspaceRoot,
								this.logger,
								bearingsConfig,
								this.bearingsExecFn,
							...
							yield {
								kind: LoopEventKind.BearingsCompleted,
								healthy: bearingsResult.healthy,
								durationMs: bearingsDuration,
								issues: bearingsResult.issues,
							};
							yield {
								kind: LoopEventKind.BearingsChecked,
								healthy: bearingsResult.healthy,
							...
							if (!bearingsResult.healthy) {
								...
								if (
									!currentPrd.includes(fixLine) &&
									!currentPrd.includes(fixLineChecked)
								) {
									fs.writeFileSync(
										prdPath,
										fixLine + "\n" + currentPrd,
						...
						yield {
							kind: LoopEventKind.BearingsSkipped,
							reason: `level is 'none' for ${!startupBearingsDone ? "startup" : "per-task"}`,
						};
					}
					if (!startupBearingsDone) {
						startupBearingsDone = true;
				...
				try {
					appendProgress(
						progressPath,
						`[${taskInvocationId}] [${task.taskId}] Task started: ${task.description}`,
					);
					...
					const relevantLearnings = knowledgeManager
						? knowledgeManager.getRelevantLearnings(
								this.config.workspaceRoot,
								task.description,
							)
						: [];
					...
					if (operatorContext) {
						this.onEvent({
							kind: LoopEventKind.ContextInjected,
					...
					const monitorConfig =
						this.config.parallelMonitor ?? DEFAULT_PARALLEL_MONITOR;
					const monitorSignals: MonitorSignals = {
						getPrdMtime: () => {
							try {
								return fs.statSync(prdPath).mtimeMs;
							} catch {
								return 0;
							}
						},
						...
						getProgressSize: () => {
							try {
								return fs.statSync(progressPath).size;
							} catch {
								return 0;
							}
						},
					...
					let execResult;
					try {
						execResult = await this.executionStrategy.execute(
							task,
							prompt,
							this.executionOptionsForTask(task),
						);
					...
					monitor.stop();
					yield {
						kind: LoopEventKind.CopilotTriggered,
						method: execResult.method,
						taskInvocationId,
					};
					yield {
						kind: LoopEventKind.WaitingForCompletion,
						task,
						taskInvocationId,
					};
					let waitResult = {
						completed: execResult.completed,
						hadFileChanges: execResult.hadFileChanges,
					};
					...
					while (
						...
						!this.stopRequested &&
						taskState.nudgeCount < this.config.maxNudgesPerTask
					) {
						...
						if (cbResult.tripped) {
							this.logger.log(
								`Circuit breaker tripped before nudge: ${cbResult.reason}`,
							);
							yield {
								kind: LoopEventKind.CircuitBreakerTripped,
								breakerName: "",
						...
						if (waitResult.hadFileChanges || task.noDiff) {
							if (waitResult.hadFileChanges) {
								this.logger.log(
									"Productive file changes detected — resetting nudge count",
								);
							}
						...
						yield {
							kind: LoopEventKind.TaskNudged,
							task,
							nudgeCount: taskState.nudgeCount,
							taskInvocationId,
						};
						this.logger.log(
							`Nudging task (${taskState.nudgeCount}/${this.config.maxNudgesPerTask}): ${task.description}`,
						);

						const finalNudge = buildFinalNudgePrompt(
							task.description,
							taskState.nudgeCount,
							this.config.maxNudgesPerTask,
						);
						...
						const nudgeResult = await this.executionStrategy.execute(
							task,
							nudgePrompt,
							this.executionOptionsForTask(task),
						);
						waitResult = {
							completed: nudgeResult.completed,
							hadFileChanges: nudgeResult.hadFileChanges,
						};
					}
					...
					if (stagnationDetector) {
						...
						if (stagnation.staleIterations > 0) {
							yield {
								kind: LoopEventKind.StagnationDetected,
								task,
								staleIterations: stagnation.staleIterations,
								filesUnchanged: stagnation.filesUnchanged,
							};
						}
						...
						if (stagnation.staleIterations >= stagnationThreshold + 2) {
							...
							yield {
								...
								task,
								reason:
									"Stagnation detected — no progress after multiple attempts",
								failCount: stagnation.staleIterations,
								taskInvocationId,
							};
							this.pauseRequested = true;
							while (this.pauseRequested) {
							...
							const depTaskId = await analyzeMissingDependency(
								task,
								progressContent,
								prdPath,
							);
							...
							yield {
								kind: LoopEventKind.CircuitBreakerTripped,
								breakerName: "stagnation",
								reason: "Stagnation detected — progress files unchanged",
								action: "skip",
								taskInvocationId,
							};
							...
							additionalContext =
								"You appear to be stuck. Progress file has not changed. Try a different approach.";
						}
					}
					...
					if (struggleDetector) {
						...
						if (struggle.struggling) {
							...
							yield {
								kind: LoopEventKind.StruggleDetected,
					...
					if (gateResult.canComplete) {
						...
						appendProgress(
							progressPath,
							`[${taskInvocationId}] [${task.taskId}] Task completed: ${task.description} (${Math.round(duration / 1000)}s)`,
						);
						yield {
							kind: LoopEventKind.TaskCompleted,
							task: { ...task, status: TaskStatus.Complete },
						...
						if (knowledgeManager) {
							let capturedOutput = "";
							try {
								capturedOutput = fs.readFileSync(progressPath, "utf-8");
							} catch {
								/* may not exist */
							}
							const learnings =
								knowledgeManager.extractLearnings(capturedOutput);
							const gaps = knowledgeManager.extractGaps(capturedOutput);
							if (learnings.length > 0 || gaps.length > 0) {
								knowledgeManager.persist(
									this.config.workspaceRoot,
									learnings,
									gaps,
								);
						...
						if (this.consistencyChecker) {
							...
							const ccResult =
								await this.consistencyChecker.runDeterministic(ccInput);
							if (ccResult.passed) {
								this.onEvent({
									kind: LoopEventKind.ConsistencyCheckPassed,
									phase: "post_task",
									checks: ccResult.checks,
								});
							} else {
								this.onEvent({
									kind: LoopEventKind.ConsistencyCheckFailed,
									phase: "post_task",
									checks: ccResult.checks,
									failureReason: ccResult.failureReason,
								});
								this.logger.warn(
									`Consistency check failed: ${ccResult.failureReason}`,
								);
							}
						}
						...
						const diffConfig =
							this.config.diffValidation ?? DEFAULT_DIFF_VALIDATION;
						if (diffConfig.enabled && !task.noDiff) {
							...
							while (diffAttempt < this.config.maxDiffValidationRetries) {
								const diffResult = await diffValidator.validateDiff(
									this.config.workspaceRoot,
									taskInvocationId,
								);
								...
								if (diffResult.hasDiff) {
									...
									if (diffConfig.generateSummary) {
										await diffValidator.appendStateToProgress(
											progressPath,
											task.id,
											diffResult,
										);
								...
								const nudge =
									diffResult.nudge ??
									"No code changes detected. Review the task requirements and make the necessary code modifications.";
								yield {
									kind: LoopEventKind.DiffValidationFailed,
									task,
									nudge,
									attempt: diffAttempt,
									taskInvocationId,
								...
								this.logger.log(
									`Diff validation failed (attempt ${diffAttempt}/${this.config.maxDiffValidationRetries}): re-entering task with nudge`,
								);
								...
								try {
									progressContentRetry = fs.readFileSync(progressPath, "utf-8");
								} catch {
									/* may not exist */
								}
								...
								retryPrompt += "\n\n" + nudge;
								const retryExec = await this.executionStrategy.execute(
									task,
									retryPrompt,
									this.executionOptionsForTask(task),
								);
								waitResult = {
									completed: retryExec.completed,
									hadFileChanges: retryExec.hadFileChanges,
								};
							}
						...
						const diffForConfidence:
							| import("./types").DiffValidationResult
							| undefined = waitResult.hadFileChanges
							? {
									filesChanged: [],
									linesAdded: 0,
									linesRemoved: 0,
						...
						yield {
							kind: LoopEventKind.ConfidenceScored,
						...
						if (confidence.score < confidenceThreshold) {
							const failing = Object.entries(confidence.breakdown)
								.filter(([, v]) => v === 0)
								.map(([k]) => k)
								.join(", ");
						...
						const preCompleteHooks =
							this.config.preCompleteHooks ?? DEFAULT_PRE_COMPLETE_HOOKS;
						...
						if (completeHook.additionalContext) {
							additionalContext = completeHook.additionalContext;
						}
						if (completeHook.blocked) {
							...
							yield {
								kind: LoopEventKind.CommandBlocked,
								command: this.config.hookScript ?? "unknown",
								reason: completeHook.reason ?? "unknown",
								taskId: task.taskId,
							};
						...
						const reviewConfig =
							this.config.reviewAfterExecute ?? DEFAULT_REVIEW_AFTER_EXECUTE;
						if (reviewConfig.enabled) {
							...
							appendProgress(
								progressPath,
								`[${taskInvocationId}] Review verdict: ${verdict.outcome} — ${verdict.summary}`,
							);
							yield {
								kind: LoopEventKind.TaskReviewed,
								task,
								verdict,
								taskInvocationId,
							};
						...
						if (!prdEditValidation.allowed) {
							this.logger.warn(
								`PRD write protection: ${prdEditValidation.reason}`,
							);
							...
							yield {
								kind: LoopEventKind.Error,
								message: `PRD write protection: ${prdEditValidation.reason}`,
							};
						}
						...
						if (commitResult.success) {
							appendProgress(
								progressPath,
								`[${taskInvocationId}] Committed: ${commitResult.commitHash}`,
							);
							yield {
								kind: LoopEventKind.TaskCommitted,
								task,
							...
							yield {
								kind: LoopEventKind.Error,
								message: `Atomic commit failed: ${commitResult.error}`,
							};
						}
						...
						appendProgress(
							progressPath,
							`[${taskInvocationId}] [${task.taskId}] Task timed out: ${task.description} (${Math.round(duration / 1000)}s)`,
						);
						yield {
							kind: LoopEventKind.TaskTimedOut,
							task: { ...task, status: TaskStatus.TimedOut },
						...
						if (autoDecomposer) {
							...
							if (
								autoDecomposer.shouldDecompose(
									String(task.id),
									count,
									autoDecomposeConfig.failThreshold,
								)
							) {
								const currentPrdContent = readPrdFile(prdPath);
								const updatedPrd = autoDecomposer.decomposeTask(
									task,
									currentPrdContent,
								);
								fs.writeFileSync(prdPath, updatedPrd, "utf-8");
								const subTaskLines = updatedPrd
									.split("\n")
									.filter(
										(l) => l.includes("Sub-task:") && /^\s*- \[ \]/.test(l),
									)
									.map((l) => l.trim());
								yield {
									kind: LoopEventKind.TaskDecomposed,
									originalTask: task,
						...
						const failHook = await this.hookService.onTaskComplete({
							taskId: String(task.id),
							result: "failure",
							taskInvocationId,
						});
						if (failHook.additionalContext) {
							additionalContext = failHook.additionalContext;
						}
						if (failHook.blocked) {
							...
							yield {
								kind: LoopEventKind.CommandBlocked,
								command: this.config.hookScript ?? "unknown",
								reason: failHook.reason ?? "unknown",
								taskId: task.taskId,
							};
							continue;
					...
					while (this.shouldRetry(currentError, retryCount)) {
						...
						if (cbResult.tripped) {
							this.logger.log(
								`Circuit breaker tripped before retry: ${cbResult.reason}`,
							);
							yield {
								kind: LoopEventKind.CircuitBreakerTripped,
								breakerName: "",
						...
						retryCount++;
						yield {
							kind: LoopEventKind.TaskRetried,
							task,
							retryCount,
							taskInvocationId,
						};
						this.logger.log(
							`Retrying task (${retryCount}/${MAX_RETRIES_PER_TASK}): ${task.description}`,
						);
						...
						try {
							...
							const retryExecResult = await this.executionStrategy.execute(
								task,
								prompt,
								this.executionOptionsForTask(task),
							);
							yield {
								kind: LoopEventKind.CopilotTriggered,
								method: retryExecResult.method,
								taskInvocationId,
							};

							const retryResult = {
								completed: retryExecResult.completed,
								hadFileChanges: retryExecResult.hadFileChanges,
							};
							...
							if (retryResult.completed) {
								...
								appendProgress(
									progressPath,
									`[${taskInvocationId}] [${task.taskId}] Task completed (after ${retryCount} retries): ${task.description} (${Math.round(duration / 1000)}s)`,
								);
								yield {
									kind: LoopEventKind.TaskCompleted,
									task: { ...task, status: TaskStatus.Complete },
					...
					if (!handled) {
						...
						appendProgress(
							progressPath,
							`[${taskInvocationId}] [${task.taskId}] Task error: ${task.description} — ${message}`,
						);
						yield {
							kind: LoopEventKind.Error,
							message: `Task "${task.description}" failed: ${message}`,
						};

						// TaskComplete hook (failure after retries exhausted)
						const errorHook = await this.hookService.onTaskComplete({
							taskId: String(task.id),
							result: "failure",
							taskInvocationId,
						});
						if (errorHook.additionalContext) {
							additionalContext = errorHook.additionalContext;
						}
						if (errorHook.blocked) {
							...
							yield {
								kind: LoopEventKind.CommandBlocked,
								command: this.config.hookScript ?? "unknown",
								reason: errorHook.reason ?? "unknown",
								taskId: task.taskId,
							};
				...
				if (this.config.cooldownShowDialog !== false) {
					...
					if (dialogResult === "edit") {
						const userInput = await vscode.window.showInputBox({
							prompt: "Provide context for the next task",
						});
						if (userInput) {
...
export function loadConfig(workspaceRoot: string): RalphConfig {
	...
	const features = {
		useHookBridge: featConfig.get<boolean>(
			"useHookBridge",
			DEFAULT_FEATURES.useHookBridge,
		),
		useSessionTracking: featConfig.get<boolean>(
			"useSessionTracking",
			DEFAULT_FEATURES.useSessionTracking,
		),
		useAutopilotMode: featConfig.get<boolean>(
			"useAutopilotMode",
			DEFAULT_FEATURES.useAutopilotMode,
		),
		useParallelTasks: featConfig.get<boolean>(
			"useParallelTasks",
			DEFAULT_FEATURES.useParallelTasks,
		),
		useLlmConsistencyCheck: featConfig.get<boolean>(
			"useLlmConsistencyCheck",
			DEFAULT_FEATURES.useLlmConsistencyCheck,
		),
	};

	return {
		prdPath: vsConfig.get<string>("prdPath", DEFAULT_CONFIG.prdPath),
		progressPath: vsConfig.get<string>(
			"progressPath",
			DEFAULT_CONFIG.progressPath,
		),
		maxIterations: vsConfig.get<number>(
			"maxIterations",
			DEFAULT_CONFIG.maxIterations,
		),
		hardMaxIterations: vsConfig.get<number>(
			"hardMaxIterations",
			DEFAULT_CONFIG.hardMaxIterations,
		),
		countdownSeconds: vsConfig.get<number>(
			"countdownSeconds",
			DEFAULT_CONFIG.countdownSeconds,
		),
		inactivityTimeoutMs: vsConfig.get<number>(
			"inactivityTimeoutMs",
			DEFAULT_CONFIG.inactivityTimeoutMs,
		),
		maxNudgesPerTask: vsConfig.get<number>(
			"maxNudgesPerTask",
			DEFAULT_CONFIG.maxNudgesPerTask,
		),
		hookScript: vsConfig.get<string | undefined>("hookScript", undefined),
		executionStrategy: vsConfig.get<"command" | "api">(
			"executionStrategy",
			DEFAULT_CONFIG.executionStrategy,
		),
		promptBlocks: vsConfig.get<string[]>(
			"promptBlocks",
			DEFAULT_CONFIG.promptBlocks!,
		),
		...
		maxParallelTasks: vsConfig.get<number>(
			"maxParallelTasks",
			DEFAULT_CONFIG.maxParallelTasks,
		),
		workspaceRoot,
		diffValidation: vsConfig.get(
			"diffValidation",
			DEFAULT_CONFIG.diffValidation,
		),
		maxDiffValidationRetries: vsConfig.get<number>(
			"maxDiffValidationRetries",
			DEFAULT_CONFIG.maxDiffValidationRetries,
		),
		reviewAfterExecute: vsConfig.get(
			"reviewAfterExecute",
			DEFAULT_CONFIG.reviewAfterExecute,
		),
		maxConcurrencyPerStage: vsConfig.get<number>(
			"maxConcurrencyPerStage",
			DEFAULT_CONFIG.maxConcurrencyPerStage,
		),
		parallelMonitor: vsConfig.get(
			"parallelMonitor",
			DEFAULT_CONFIG.parallelMonitor,
		),
		preCompactBehavior: vsConfig.get(
			"preCompactBehavior",
			DEFAULT_CONFIG.preCompactBehavior,
		),
		stagnationDetection: vsConfig.get(
			"stagnationDetection",
			DEFAULT_CONFIG.stagnationDetection,
		),
		...
		contextTrimming: vsConfig.get(
			"contextTrimming",
			DEFAULT_CONFIG.contextTrimming,
		),
		bearings: vsConfig.get("bearings", DEFAULT_CONFIG.bearings),
		cooldownShowDialog: vsConfig.get<boolean>(
			"cooldownShowDialog",
			DEFAULT_CONFIG.cooldownShowDialog ?? true,
		),
