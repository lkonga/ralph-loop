...
export function buildPrompt(taskDescription: string, prdContent: string, progressContent: string, maxProgressLines: number = 20, promptBlocks?: string[], capabilities?: PromptCapabilities, learnings?: string[], iterationNumber: number = 1, contextTrimming?: ContextTrimmingConfig, operatorContext?: string, taskId?: string, promptTemplate?: string, workspaceRoot?: string): string {
	...
	const parts: string[] = [
		...
		'===================================================================',
		'',
		'Every task MUST follow the TDD cycle: (1) Write a failing test FIRST that covers the acceptance criteria, (2) Run the project test suite \u2014 confirm the new test FAILS (red), (3) Implement the minimum code to make the test pass, (4) Run the test suite again \u2014 confirm ALL tests PASS (green), (5) Refactor if needed \u2014 run tests again to confirm still green, (6) Run the build check \u2014 must exit with zero errors, (7) ONLY THEN mark the checkbox in PRD.md. If ANY test is red or the build reports errors, you are NOT done \u2014 fix them before proceeding. This is not optional. No checkbox may be marked while tests are failing. Use `createAndRunTask` to run tests and `testFailure` for structured error details. Check `.ralph/run-config.json` for the configured test/build commands.',
		'',
		'===================================================================',
